...
public class Kmeans extends Common
{
	...
	public ClustersAndTheirStatistics assignPointsToClustersAndUpdateCentres(Cluster[] clusters, Cluster points, boolean isRecalculateCentres) {
		...
		if(isRecalculateCentres) {
			clusters = updateClusterCenters(clusters); // TODO: moze nie musze update robic centrum? bo moze nam wedrowac
		}

		clusters = removeEmptyClusters(clusters);
		return new ClustersAndTheirStatistics(clusters, measure.calculateClusterisationStatistic(clusters), true);
	}
