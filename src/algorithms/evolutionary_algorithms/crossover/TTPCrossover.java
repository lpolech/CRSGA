...
public class TTPCrossover extends BaseCrossover<Integer, BaseProblemRepresentation> {
  ...
  public List<List<Integer>> crossover(double cr, List<Integer> firstParent, List<Integer> secondParent, ParameterSet<Integer, BaseProblemRepresentation> parameters) {
    ...
    if(parameters.TSPcrossoverVersion == 1) {
      ...
      intermediateResult = indWiseEdgeCrossoverTSP(cr, firstParent, secondParent, parameters, firstChild, secondChild);
    } else if(parameters.TSPcrossoverVersion == 2) {
      intermediateResult = indWiseTwoOrSinglePointPMXCrossoverTSP(cr, firstParent, secondParent, parameters, parameters.geneSplitPoint, false); // TWO point as the last param is true
    } else if(parameters.TSPcrossoverVersion == 3) {
      intermediateResult = indWiseTwoOrSinglePointPMXCrossoverTSP(cr, firstParent, secondParent, parameters, parameters.geneSplitPoint, true); // SINGLE point as the last param is true
    } else if(parameters.TSPcrossoverVersion == 4) {
      intermediateResult = indWiseOXCrossoverTSP(cr, firstParent, secondParent, parameters, parameters.geneSplitPoint, false);
    } else if(parameters.TSPcrossoverVersion == 5) {
      intermediateResult = indWiseOXCrossoverTSP(cr, firstParent, secondParent, parameters, parameters.geneSplitPoint, true);
    } else if(parameters.TSPcrossoverVersion == 6) {
      intermediateResult = indWiseCXCrossoverTSP(cr, firstParent, secondParent, parameters, parameters.geneSplitPoint); // BASELINE
    }
...
//    printTSPGeneDifference(firstParent, secondParent, intermediateResult.firstChild(), intermediateResult.secondChild(), parameters.geneSplitPoint);
    ...
    if(parameters.KNAPcrossoverVersion == 1) {
      indWiseUniformCrossoverKNAP(parameters.KNAPcrossoverProbability, firstParent, secondParent, parameters, intermediateResult); // BASELINE
    } else if(parameters.KNAPcrossoverVersion == 2) {
      indWiseSinglePointCrossoverKNAP(parameters.KNAPcrossoverProbability, firstParent, secondParent, parameters, intermediateResult);
    } else if(parameters.KNAPcrossoverVersion == 3) {
      indWiseTwoPointCrossoverKNAP(parameters.KNAPcrossoverProbability, firstParent, secondParent, parameters, intermediateResult);
    }
  ...
  private List<Integer> getChild(List<Integer> child, List<Integer> firstParent, List<Integer> secondParent,
    ...
    availableGenes.remove(Integer.valueOf(x));
